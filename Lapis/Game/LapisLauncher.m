    ...
    NSString * _Nullable delegateClassName)
{
    ...
    if (existing != nil) {
        NSLog(@"[Lapis:Guard] Blocked UIApplicationMain — JVM context active");
        dispatch_async(dispatch_get_main_queue(), ^{
            [[NSNotificationCenter defaultCenter] 
                postNotificationName:@"LapisJVMContextReady" object:nil];
        });
        while(1) { sleep(1); }
        return 0;
    }
...
static void setupDefaultEnvironment(void) {
    ...
    setenv("GLFW_NSGL_HYBRID_GRAPHICS", "1", 1);
    setenv("org.lwjgl.opengl.Display.noinput", "true", 1);
...
int LapisEngine_launchJVM(NSArray<NSString *> *args) {
    @autoreleasepool {
        ...
        // STEP 1: Install hook FIRST, before loading JRE
